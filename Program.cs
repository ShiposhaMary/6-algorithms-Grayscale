...
namespace Recognizer
{
    internal static class Program
    {
        ...
        private static void Main()
        {
            ...
            var grayscale = My_GrayscaleTask.ToGrayscale(pixels);
            panel.Controls.Add(CreateBox(ConvertToBitmap(grayscale)), 1, 0);
            var clear = My_MedianFilterTask.MedianFilter(grayscale);
            panel.Controls.Add(CreateBox(ConvertToBitmap(clear)), 2, 0);
            var sobell = My_SobelFilterTask.SobelFilter(clear, new double[,] {{-1, -2, -1}, {0, 0, 0}, {1, 2, 1}});
            panel.Controls.Add(CreateBox(ConvertToBitmap(sobell)), 0, 1);
            var trashhold = My_ThresholdFilterTask.ThresholdFilter(sobell, 0.1);
